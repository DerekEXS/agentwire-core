...
| `auth.token` | `16a40d2a51fc383c542c62dc0d494db6f4c28ed6040ae391` | **gateway auth token** |
| `port` | `18789` | — |
...
```json
{
  ...
  "auth": {
    "token": "16a40d2a51fc383c542c62dc0d494db6f4c28ed6040ae391"
  },
  "role": "operator",
...
**方案 C 的关键绕行点**：以 WS client 身份连接时，`scopes` 在 connect 请求中声明。OpenClaw 的 scope 限制是针对 **plugin runtime** 的（`canTrustedOfficialPluginRequestScopes`），不是针对 WS client 的。CLI 以 `role:"operator"` + `scopes:["operator.write"]` 连接 WS，能成功调 agent（前面实测初梦在线）。**WS client 的身份不受 plugin trust gate 限制**。
...
| 认证 | **已验证可用**（实测 CLI 成功调初梦）——全自动由 CLI 处理（token、设备签名、scope 全在内） |
...
理由：
1. C2 已实锤可行（CLI 调初梦成功），零协议风险。C1 的设备签名未决（可能需要额外实现 `loadOrCreateDeviceIdentity` + Ed25519 签名 = 额外 100+ 行 + crypto 依赖）。
...
const GW_TOKEN = "16a40d2a51fc383c542c62dc0d494db6f4c28ed6040ae391";
...
| 认证方式 | `auth.mode=token`，token `16a40d2a51fc...` 已知。connect 请求 `auth.token` 字段传此值 |

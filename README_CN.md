...
- **A2A v1.0** —— 基于官方 [a2a-sdk](https://pypi.org/project/a2a-sdk/) v1.1.0+
  - 标准 Agent Card 同时暴露在 `/.well-known/agent.json` 和 `/.well-known/agent-card.json`
  - 标准方法:PascalCase `SendMessage`/`GetTask`/`ListTasks`/`CancelTask`/`SendStreamingMessage`
  - Task 状态机: `SUBMITTED → WORKING → COMPLETED/FAILED/CANCELED`
- **HTTP 上的 JSON-RPC 2.0** —— 标准、语言无关的传输 (`/a2a/jsonrpc`)
- **三级 Agent 路由** —— `agentId` 显式优先 → YAML 规则 (tags/patterns/priority) → 默认回退 (v2.0)
- **SQLite TaskStore** —— 持久化 Task 生命周期,带索引查询
...
- **遗留兼容**:v2.0 在端口 18800 上仍可用 v1.x `messages/*` JSON-RPC 方法与 `/a2a/rest/message/send` 端点,过渡期 agent 可直接平移
...
把 `dist/` 放到你的 OpenClaw 插件加载器里。Manifest 已适配 OpenClaw 2026.6.6 schema:`additionalProperties: false`、`kind: "http-server"`、`uiHints` 全字段标注、`version: "2.0.1"` 与 CORE 镜像 tag 对齐。
...
| 方法 | 路径 | 鉴权 | 用途 |
|------|------|------|------|
| GET | `/.well-known/agent.json` | 可选 | Agent Card 发现(主路径, v2.0.1+) |
| GET | `/.well-known/agent-card.json` | 可选 | Agent Card 发现(SDK 兼容路径, 内容相同) |
| GET | `/health` | 无 | 存活探针 |
| GET | `/a2a/metrics` | Bearer (v1.5.2+) | 任务计数 |
| GET | `/redact/patterns` | 必需 | 共享脱敏 pattern 目录 |
| POST | `/a2a/jsonrpc` | 必需 | JSON-RPC 2.0 端点(承载全部标准 A2A 方法) |

## JSON-RPC 方法

标准 A2A v1.0 (v2.0+):

- `SendMessage` —— 发消息,返回 Task
- `GetTask` —— 按 ID 取 Task
- `ListTasks` —— 列全部 Task,支持分页
- `CancelTask` —— 取消 Task
- `SendStreamingMessage` —— SSE 流式任务
- `GetExtendedAgentCard` —— 取扩展 Agent Card

遗留兼容(v1.5.x server 仍支持,新 agent 不应使用):
...
CORE 镜像 `agentwire-core:v2.0.1` 与 CUE 一同在 [agentwire-cue 仓的 docker-compose.yml](https://github.com/DerekEXS/agentwire-cue/blob/main/docker-compose.yml) 发布。Compose 把 CORE 跑在 `127.0.0.1:18800`、CUE 跑在 `127.0.0.1:18801` + `127.0.0.1:19000`,都仅 host loopback 发布并共享 token secret。
